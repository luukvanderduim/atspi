...
glib::glib_wrapper! {
    ...
    match fn {
        get_type => || ffi::atspi_accessible_get_type(),
    }
}
...
impl<O: IsA<Accessible>> AccessibleExt for O {
    ...
    fn set_cache_mask(&self, mask: Cache) {
        unsafe {
            ffi::atspi_accessible_set_cache_mask(self.as_ref().to_glib_none().0, mask.to_glib());
        }
    }
    ...
    fn connect_region_changed<F: Fn(&Self, i32, i32) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn region_changed_trampoline<P, F: Fn(&P, i32, i32) + 'static>(this: *mut ffi::AtspiAccessible, arg1: libc::c_int, arg2: libc::c_int, f: glib::ffi::gpointer)
            where P: IsA<Accessible>
        {
            let f: &F = &*(f as *const F);
