...
pub trait TextExt: 'static {
    ...
    #[deprecated]
    fn get_text_at_offset(&self, offset: i32, type_: TextBoundaryType) -> Result<TextRange, Error>;
...
impl<O: IsA<Text>> TextExt for O {
    ...
    fn get_text_at_offset(&self, offset: i32, type_: TextBoundaryType) -> Result<TextRange, Error> {
        unsafe {
            let mut error = ptr::null_mut();
            let ret = atspi_sys::atspi_text_get_text_at_offset(self.as_ref().to_glib_none().0, offset, type_.to_glib(), &mut error);
            if error.is_null() { Ok(from_glib_full(ret)) } else { Err(from_glib_full(error)) }
        }
    }
